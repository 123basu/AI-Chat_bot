...
def _get_pg_pool():
    ...
    if _pg_pool is None:
        dsn = DATABASE_URL
        if "localhost" not in dsn and "127.0.0.1" not in dsn and "neon" in dsn.lower():
            if "sslmode" not in dsn:
                dsn = dsn + ("" if "?" in dsn else "?") + "sslmode=require"
        _pg_pool = pool.SimpleConnectionPool(1, 20, dsn=dsn)
    return _pg_pool
...
# For backwards compatibility with start-up call
def get_conn():
